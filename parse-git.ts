...
const RAW_OUT = new TextDecoder().decode(
  await Deno.run({
    cmd: [
      "git",
      "log",
      `--pretty=format:{%n  "commit": "%H",%n  "author": {%n    "name": "%aN",%n    "email": "%aE",%n    "date": "%aD"%n  }%n },`,
    ],
    stdout: "piped",
  }).output(),
);
...
async function writeSnapshotFile(commit: TGitLogCommits[number]) {
  ...
  Deno.writeTextFileSync(
    `${OUTPUT_DIR}/${commit.commit}${FILE_EXTENSION}`,
    `{"commit": "${commit.commit}", "data": ${COMMIT_DATA}}`,
  );
}
...
for (const commit of commitHistory) {
  if (
    ...
    (IGNORE_EXISTING_FILES &&
      !(await fileExists(`${OUTPUT_DIR}/${commit.commit}${FILE_EXTENSION}`)))
  ) {
