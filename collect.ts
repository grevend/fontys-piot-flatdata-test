...
async function transform(input: Data): Promise<Collected> {
    return {...input, plants: await Promise.all(input.plants.map(async (plant, idx) => {
        if(plant.image !== undefined) {
            ...
            await exec(`squoosh-cli --max-optimizer-rounds 10 --optimizer-butteraugli-target 1.7 --webp auto plant-${idx + 1}.png`)
            await exec(`squoosh-cli --max-optimizer-rounds 10 --optimizer-butteraugli-target 1.7 --avif auto plant-${idx + 1}.png`)
        }
        return ({
